#ifndef _SKIP_DATABASE_H_
#define _SKIP_DATABASE_H_
...
public:
    ...
    void run() {
        std::thread dump_thread(Skipdb::dump_file, this);  //调用skiplist空间的存盘函数
        std::thread delete_thread(Skipdb::cycle_to_delete, this);  //选出倒数的k，然后在两个空间执行删除函数
        dump_thread.detach();
...
private:
    ...
    static void cycle_to_delete(void* arg) {
        ...
        while (!ptr->is_close) {
            ...
            while (wait_to_delete--) {
                auto key = ptr->lru->get_head_key();  //从尾部加入的节点
                // 获得的键已过期
                if (ptr->lru->is_exist(key) && ptr->lru->is_expired(key)) {
                    cout << "cycle_delete" << endl;
                    ptr->expired_key++;
                    ptr->skip_list->delete_element(key); //两个内存都需要删除
    ...
    // 查询键，查询如果键过期了，返回存在，并且在两个空间惰性删除掉
